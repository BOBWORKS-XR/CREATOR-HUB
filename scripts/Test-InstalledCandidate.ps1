param(
    ...
    [ValidateSet('clean', 'alpha.3', 'alpha.4', 'alpha.5', 'alpha.6', 'stable-0.1.0', 'stable-0.1.1', 'stable-0.1.2')][string]$HubBaseline = 'clean'
)
...
try {
    ...
    if ($HubBaseline -ne 'clean') {
        $baseline = if ($HubBaseline -eq 'stable-0.1.2') {
